...
onMounted(async () => {
  try {
    ...
    const res = await axios.get(`${API_URL}/contactos/detalle.php?id=${id}`);
    if (res.data.success) {
      Object.assign(form, res.data.contacto);
    } else {
      msg.value = 'No se pudo cargar el contacto';
    }
  } catch (e) {
    msg.value = 'Error al cargar contacto';
  }
});
...
const actualizar = async () => {
  ...
  try {
    const API_URL = await getApiUrl();
    const res = await axios.post(`${API_URL}/contactos/actualizar.php`, data, {
